...
const hashmapentry = hashfunc => x => [ hashfunc(x), x ];
...
function listdiffsymmetric(list1, list2, hashfunc) {
    ...
    return function* _generatedsymdiffedlist() {

        const differ = symdifferfactory(list2, hashfunc);

        for(const value of list1) differ.isuniqdiff(value) && (yield value);
...
function symdifferfactory(list2, hashfunc) {

    return hashfunc
         ? symdifferfactory_hashed(list2, hashfunc)
         : symdifferfactory_simple(list2);
}

function symdifferfactory_simple(list2) {

    const set1 = new Set();
    const isuniq1 = value => (set1.size < set1.add(value).size);

    const set2 = new Set(list2);
    const isdiff2 = value => !set2.delete(value);
    ...
    function isuniqdiff(value) {
        const diff = isdiff2(value);
        const uniq = isuniq1(value);
        return diff && uniq;
    }
}

function symdifferfactory_hashed(list2, hashfunc) {

    const set1 = new Set();
    const isuniq1 = hashvalue => (set1.size < set1.add(hashvalue).size)

    const hashlist2 = { [Symbol.iterator]:maplist(hashmapentry(hashfunc), list2) }
    const map2 = new Map(hashlist2);
    const isdiff2 = hashvalue => !map2.delete(hashvalue);
    ...
    function isuniqdiff(value) {
        ...
        const diff = isdiff2(hashvalue);
        const uniq = isuniq1(hashvalue);
        return diff && uniq;
    }
