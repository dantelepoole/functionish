...
const lazylist = (func, ...args) => ( new LazyList( defer(func, args), CONSTRUCTORLOCK) );
...
const raisebadgenerator = func => { throw new TypeError( format(ERR_BAD_GENERATE, typeof func) ) }
...
class LazyList {
    ...
    static from = (source, ...args) => islazylist(source) && source
                                    || isfunction(source) && lazylist(source, ...args)
                                    || isiterable(source) && lazylist(getiterator, source)
    ...
    concat(...items) {
        return lazylist(listconcat, this, items);
    }

    filter(predicate) {

        return isfunction(predicate) && lazylist(listfilter, predicate, this)
                ||
    ...
    flat(depth=1) {
        ...
        return ispositiveinteger(depth) && lazylist(listflat, depth, this)
                ||
               raisebaddepth();
    }
    ...
    flatMap(mapfunc, depth=1) {
        ...
        return isfunction(mapfunc) && ispositiveinteger(depth) && lazylist(listflatMap, depth, mapfunc, this)
                ||
    ...
    map(mapfunc) {
        ...
        return isfunction(mapfunc) && lazylist(listmap, mapfunc, this)
                ||
               raisebadmapfunction(mapfunc);
    }

    reverse() {
        return lazylist(listreverse, this);
    }

    slice(start=0, end=Infinity) {

        return (end === Infinity) ? lazylist(listslicepartial, this, casttopositiveinteger(start))
                                  : lazylist(listslicerange, this, casttopositiveinteger(start), casttopositiveinteger(end));
    }

    sort(comparefunc) {

        return isempty(arguments) && lazylist(listsort, this)
                ||
               (isfunction(comparefunc) && lazylist(listsortcustom, comparefunc, this))
                ||
