 ...
 * result. Otherwise, if only one argument is missing, return *func* bound to its arguments. Otherwise,
 * return a curried version of *func*, i.e. pre-bound to the *args*.
 * 
 * In the latter case, the returned function will have a property `arity` holding the current
...
function curryfunction(arity, func, ...args) {
    ...
    const disparity = arity - args.length;

    return (disparity < 1) ? func(...args)
         : (disparity === 1) ? func.bind(CONTEXT_NONE, ...args)
         : docurry(arity, func, args);
}

function docurry(arity, func, args) {

    const curried = curryfunction.bind(CONTEXT_NONE, arity, func, ...args);

    curried.arity = (arity - args.length);
