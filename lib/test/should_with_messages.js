...
const should_be = (comparator, message) => x => expect(x, message).equals(comparator);
const should_be_a = (type, message) => x => expect( typeorclassname(x), message ).equals(type);
const should_be_empty = message => x => expect( (x.length ?? x.size), message ).equals(0);
const should_be_false = message => x => expect(x, message).is.false;
const should_be_falsy = message => x => expect( !!x, message ).is.false;
const should_be_instance_of = message => x => expect(x, message).is.instanceof(x);
const should_be_like = (comparator, message) => x => expect(x, message).deep.equals(comparator);
const should_be_nan = message => x => expect(x, message).is.NaN;
const should_be_null = message => x => expect(x, message).is.null;
const should_be_one = message => x => expect(x, message).equals(1);
const should_be_true = message => x => expect(x, message).is.true;
const should_be_truthy = message => x => expect( !!x, message ).is.true;
const should_be_type = (type, message) => x => expect(x, message).to.be.a(type);
const should_be_undefined = message => x => expect(x, message).to.be.undefined;
const should_be_void = message => x => expect( isvoid(x), message).to.be.true;
const should_be_zero = message => x => expect(x, message).equals(0);
const should_not_be = (comparator, message) => x => expect(x, message).not.equal(comparator);
const should_not_be_a = (type, message) => x => expect(x, message).is.not.a(type);
const should_not_be_empty = message => x => expect( (x.length ?? x.size), message ).not.equals(0);
const should_not_be_false = message => x => expect(x, message).is.not.false;
const should_not_be_falsy = message => x => expect( !!x, message ).is.not.false;
const should_not_be_instance_of = message => x => expect(x, message).is.not.instanceof(x);
const should_not_be_like = (comparator, message) => x => expect(x, message).not.deep.equals(comparator);
const should_not_be_nan = message => x => expect(x, message).not.is.NaN;
const should_not_be_null = message => x => expect(x, message).is.not.null;
const should_not_be_one = message => x => expect(x, message).not.equals(1);
const should_not_be_true = message => x => expect(x, message).is.not.true;
const should_not_be_truthy = message => x => expect( !!x, message ).is.not.true;
const should_not_be_type = (type,x, message) => expect(x, message).is.not.a(type);
const should_not_be_undefined = message => x => expect(x, message).is.not.undefined;
const should_not_be_void = message => x => expect( isvoid(x), message).to.be.false;
const should_not_be_zero = message => x => expect(x, message).not.equals(0);
