...
describe(`difference()`, function() {
    ...
    it(`should return a shallow copy of list1 without duplicates if list2 is empty`,
        function () {
            expect( Array.from(difference([1,2,2,3,4],[])) ).to.be.deep.equal([1,2,3,4]);
            
            const alist = [1,2,3,4];
            const result = Array.from(difference(alist, []));
    ...
    it(`should return an empty iterable object if list1 is empty`,
        function () {
            expect( Array.from(difference([],[1,2,3,4])) ).to.be.deep.equal([]);
        }
    )

    it(`should return iterable objects with its items in the same order as list1, except for duplicates`,
        function () {
            expect( Array.from(difference([42, 23, 15, 23, 42, 1],[5,6,7,8,9])) ).to.be.deep.equal([42,23,15,1]);
        }
    )
})