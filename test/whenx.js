...
describe( 'whenx()', function() {
    ...
    it('should have a `for()` method', function() {
        expect( whenx(iseven, product, sum).for ).to.be.a('function');
    })

    describe( 'The `whenx().for()` method', function() {

        it('should pass its arguments to the condition function', function() {

            expect(iseven.callCount).to.equal(0);

            whenx(iseven, product, sum).for(42, UNIQTHING, ANOTHERTHING);

            expect(iseven.callCount).to.equal(1);
            expect(iseven.args[0]).deep.equal( [42, UNIQTHING, ANOTHERTHING] );
        })

        it('should return the true branch if the condition returns a truthy value and the true branch is a function', function() {

            const productorsum = whenx(iseven, product, sum);

            expect( productorsum.for(42) ).to.equal(product);
            
        })
        
        it('should return the false branch if the condition returns a falsy value and the false branch is a function', function() {

            const productorsum = whenx(iseven, product, sum);

            expect( productorsum.for(41) ).to.equal(sum);
            
        })

        it(`should return true branch's value as a function if it is not a function and the condition returns a truthy value`, function() {

            const alwaysuniqthing = whenx(iseven, UNIQTHING).for(42);
            
            expect(alwaysuniqthing).to.be.a('function');

            expect( alwaysuniqthing() ).to.equal(UNIQTHING);
            expect( alwaysuniqthing(ANOTHERTHING, 42, 'fubar') ).to.equal(UNIQTHING);
            
        })

        it(`should return the false branch's value as a function if it is not a function and the condition returns a falsy value`, function() {

            const alwaysanotherthing = whenx(iseven, UNIQTHING, ANOTHERTHING).for(41);
            
            expect(alwaysanotherthing).to.be.a('function');

            expect( alwaysanotherthing() ).to.equal(ANOTHERTHING);
            expect( alwaysanotherthing(UNIQTHING, 42, 'fubar') ).to.equal(ANOTHERTHING);
