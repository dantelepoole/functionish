...
describe(`issymbol()`, function() {
    ...
    it(`should return true if its argument has type symbol`,
        function () {
            expecttrue( issymbol(markersymbol) );
            expecttrue( issymbol( Symbol() ) );
            expecttrue( issymbol( Symbol.for('foobar') ) );
        }
    )

    it(`should return false if its argument has any other type`,
        function () {
            expectfalse( issymbol(null) );
            expectfalse( issymbol(undefined) );
            expectfalse( issymbol(markerobject) );
            expectfalse( issymbol(markerarray) );
            expectfalse( issymbol(42) );
            expectfalse( issymbol('foobar') );
            expectfalse( issymbol(true) );
        }
    )
})
